...
class FilterSequential(nn.Sequential):
    ...
    def forward(self, x, **kwargs):
        for layer in self:
            with FilterKwargs(layer, arg_keys=list(kwargs.keys())) as l:
                x = l(x, **kwargs)
        return x
...
class CNN(FilterSequential):
    ...
    def __init__(
        ...
        if intermediate_features is not None:
            ...
            attn_resolutions = [max_attn_res+1] * len(features)
        else:
...
class AutoEncoder(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.latent_size = utils.LatentSize(
            c=latent_features * (1 + int(double_encoded_features)),
            h=latent_resolution, w=latent_resolution
        )
    ...
    def forward(self, pixel_values: Tensor, labels: Optional[Tensor] = None, time: Optional[Tensor] = None) -> AEOutput:
        latent = self.encode(pixel_values, labels, time)
        preds = self.decode(latent, labels, time)
